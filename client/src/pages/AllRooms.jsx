...
const AllRooms = () => {
  ...
  const matchesPriceRange = (room) => (
    ...
    selectedFilters.priceRange.some(range => {
      const [min, max] = range.replace(/[^0-9 to]/g, '').split(' to ').map(Number)
    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      <div className='w-full'>
        ...
        {filteredRooms.length === 0 ? (
          <div className='text-gray-400 text-center py-16'>No rooms found for your filters/search.</div>
        ) : (
          filteredRooms.map((room) => (
            <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
              <img
                onClick={() => { if(room._id) { navigate(`/rooms/${room._id}`); window.scrollTo(0, 0); } }}
                src={room.images?.[0] || assets.fallbackHotelImage}
                alt="hotel-img"
                title='View Room Details'
                className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'
                onError={e => { e.target.src = assets.fallbackHotelImage }}
              />
              <div className='md:w-1/2 flex flex-col gap-2'>
                <p className='text-gray-500'>{room.hotel?.city ?? '—'}</p>
                <p onClick={() => { if(room._id) { navigate(`/rooms/${room._id}`); window.scrollTo(0, 0); } }}
                ...
                <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                  {Array.isArray(room.amenities) && room.amenities.map((item) => (
      ...
      <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
        ...
        <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`}>
          <div className='px-5 pt-5'>
            ...
            {roomTypes.map((room) => (
              <CheckBox key={room} label={room} selected={selectedFilters.roomType.includes(room)}
          ...
          <div className='px-5 pt-5 pb-7'>
            ...
            {sortOptions.map((option) => (
              <RadioButton key={option} label={option} selected={selectedSort === option}
